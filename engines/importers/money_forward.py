"""
engines/importers/money_forward.py

マネーフォワード クラウド会計の仕訳データを FAC フォーマットに分解するインポーター。
...
# --------------------------------------------------
# インポーターの自己申告メタ情報
# GUI/レジストリはこれを見て選択肢を自動生成する
# --------------------------------------------------
IMPORTER_META = {
    "id": "money_forward",
    "display_name": "マネーフォワード クラウド会計",
    "required_encoding": "cp932",
}

# 仕訳データ側で必須となる列(リネーム後の名前で判定)
_REQUIRED_MF_COLUMNS = [
    "取引日",
    "借方勘定科目",
    "借方部門",
    "借方金額",
...
def decompose_to_fac(source_path: str, mapping_path: str) -> pd.DataFrame:
    ...
    Args:
        source_path: マネーフォワードから出力した仕訳帳CSVのパス
    ...
    """
    print("=== FACフォーマット変換処理を開始します (マネーフォワード) ===")
    ...
    df_mf = pd.read_csv(source_path, encoding="cp932")
    ...
    _validate_columns(df_mf, _REQUIRED_MF_COLUMNS, source_label="仕訳データCSV")
    ...
    df_cf_in['dept_original'] = df_cf_in['貸方部門']       # 相手科目側（貸方）に統一
    df_cf_in['account_small'] = df_cf_in['貸方補助科目']    # 相手科目側の補助科目
    ...
    df_cf_out['dept_original'] = df_cf_out['借方部門']      # 相手科目側（借方）に統一
    df_cf_out['account_small'] = df_cf_out['借方補助科目']   # 相手科目側の補助科目
