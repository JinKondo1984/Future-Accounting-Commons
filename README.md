...
## 📊 FAC Format Specifications (Ver.1.0)
...
| 4 | `amount` | 数値 | **PLは管理会計用税抜、CFは税込金額**（正＝入金・収益、負＝出金・費用） |
| 5 | `cost_type` | 文字列 | 固変区分（CVP分析用）：固定費 / 変動費 / 対象外 |
| 6 | `cf_type` | 文字列 | CF区分（資金繰り用）：営業CF / 投資CF / 財務CF / 対象外 |
| 7 | `dept_original` | 文字列 | 会計ソフトから出力された時点の初期部門名。 |
| 8 | `status` | 文字列 | データ状態：実績 / 予算 / 予測 |
